...
def save_data(input_path,OUT_STAK,name):
    for i in range(len(OUT_STAK)):
        b=name.split('/')
        c=b.pop()
        d=c.split('.')
        cv2.imwrite(input_path+d[0][0:2]+str(int(d[0][2:5])+1)+'.png', OUT_STAK) #Creare cartelle 


def register(fix, mov, metric, optimizer,input_path,moving_image,j):
    t=0
    OUT_STAK=[]
    ...
    if optimizer=="gd":
        ...
        registration_method.SetOptimizerAsOnePlusOneEvolutionary(numberOfIterations=100, epsilon=1.5e-4, initialRadius=1.01, growthFactor=-1.05, shrinkFactor=-0.99,seed=12345)
    elif optimizer=="pow":
        registration_method.SetOptimizerAsPowell(numberOfIterations=100, maximumLineIterations=100, stepLength=1, stepTolerance=1e-06, valueTolerance=0.00005)
    else:
        return -100
    #
    #
    ...
    save_data(input_path,OUT_STAK,j)
    return t
...
def compute_wrapper(args, num_threads=1):
    ...
    for i,j in zip(CT, PET):
        print(i)
        print(j)
        fixed_image = sitk.ReadImage(i, sitk.sitkFloat32)
        moving_image = sitk.ReadImage(j, sitk.sitkFloat32) 
        f=sitk.GetArrayFromImage(fixed_image)
        m=sitk.GetArrayFromImage(moving_image)
        fix=sitk.GetImageFromArray(f[0])
        mov=sitk.GetImageFromArray(m[0])
        t=register(fix, mov, args.metric, args.optimizer,curr_res,moving_image,j)
        times.append(t)
